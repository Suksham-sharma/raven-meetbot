...
// DEEPGRAM_API_KEY lives in the repo-root .env.
dotenv.config();
dotenv.config({ path: path.resolve(process.cwd(), "..", ".env") });

import { diarizeRecording, serializeNamedTranscript } from "./pipeline";

// Manual/backfill CLI over local files; the BullMQ worker runs the same pipeline
// against R2.
//
//   tsx src/diarize/runDiarize.ts <recording.webm> [out.jsonl]
