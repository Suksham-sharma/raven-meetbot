...
// Build a timeline where contributor 5001 is active 0–10s and 6002 is active 10–20s
// (epoch ms = REC_START + seconds*1000). Only 5001 is bound (→ Alice).
...
describe("mergeNames", () => {
  it("names a bound speaker via the bind and the other by elimination", () => {
    ...
    // Each window sits squarely on one contributor → full confidence.
    expect(named.every((n) => n.confidence === 1)).toBe(true);
  });

  it("falls back to Speaker N when no participant name is left", () => {
    // Two unbound speakers but only one spare name (no binds at all).
    ...
    expect(assignments.find((a) => a.speaker === 0)!.name).toBe("Bob"); // elimination
