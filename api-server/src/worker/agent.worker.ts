...
import { proposeActions } from "../actions/propose";
import type { AgentJob } from "../lib/queueManager";

// v3 agent worker: one job = propose actions for one ingested meeting.
// Proposals are idempotent (action_hash), so BullMQ retries are safe. Nothing
// external fires here — execution happens only via the approve endpoint.
//
