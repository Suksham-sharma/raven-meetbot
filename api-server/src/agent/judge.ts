...
// LLM-as-judge for answer quality (the semantic half of the eval — the keyword
// fact-coverage proxy can't see paraphrase or hallucination). Same technique
// Ragas uses, implemented directly so the eval has no heavy Python dependency:
//   - faithfulness:  decompose the answer into atomic claims, mark each as
//                    supported by the retrieved contexts or not. score = supported/total.
//                    This is the hallucination detector.
//   - relevancy:     does the answer actually address the question? full/partial/none.
// Deterministic (temperature 0). Judge model is swappable + ideally != the model
// under test (config OPENAI_JUDGE_MODEL) to limit self-preference bias.
...
export interface FaithfulnessResult {
  score: number; // supported / total (1.0 if no factual claims)
  supported: number;
  total: number;
  unsupported: string[];
}
