...
// Retrieval baseline over the golden set. Now CHUNK-level: each question labels
// the chunks (meetingId#seq) that actually answer it, and we score recall@k /
// MRR / nDCG@k against the ranked chunk list hybrid search returns. Chunk-level
// is where ranking quality (and a future reranker) actually shows up — the old
// meeting-level scoring saturated at ~1.0. Meeting-level recall is kept as a
// secondary column for comparison. (Answer quality / Ragas is run_eval.py.)
//
