...
| **Step 4 — eval-gated routing + codex-reviewed eval hardening (2026-06-24)** | ✅ spine-first routing (date-aware tolerant `list_meetings` discovery + exact scoped tools + hard list-cap); judge-context fidelity; agent-path ruler (`context_fact_recall` + contamination + precision + variance); bounded `search_structured`; corpus grown to **9 meetings / 27 Q** with adversarial same-title/cross-entity/recency cases. Eval now a sharp diagnostic — exposes a gpt-4o-mini reliability ceiling on the hardest disambiguation/refusal cases |
| Dashboard | deferred slice |

**OpenAI key** lives in `api-server/.env` (gitignored). **Next action (candidates):** stronger ask-model experiment (gpt-4o / gpt-5-mini-class) to test the disambiguation ceiling, then an N=15–20 stable run (both billed tool-use — cost-gated) ‖ fix q17 contamination false-positive + per-call-type token logging (cheap) ‖ dashboard (demo) ‖ v3 agentic action-taking ‖ real-bot ingest path (R2 + clock-skew verify, still the one CRITICAL gap). Contextual retrieval/reranker **deferred** — the diagnosis showed the real gaps are routing/aggregation, not chunk ranking. See the Step-4 slice notes below.
...
- **Citation clock skew (CRITICAL gap):** storage half done — `recording_offset_s` column on `meetings` (0 for synthetic seeds). The **verify-origin half is still TODO** at real-bot ingest (compare Deepgram transcript t=0 vs recording t=0 from R2, set the offset). Citation builder in `/ask` must read it.
  - **Investigated 2026-06-24 (DEFERRED — not built).** Findings from a read of `bot/`: (1) the real `{meetingId}.transcript.jsonl` shape MATCHES the seeds (`{speaker,text,start,end,confidence}` + an extra `isFinal`, seconds from Deepgram stream start) — so the R2 ingest is a clean swap into `ingestMeeting`. (2) R2 keys are FLAT: `{meetingId}.webm` / `.transcript.jsonl` / `.speakers.jsonl`. (3) **The skew CANNOT be computed from current artifacts:** `speakers.jsonl`'s `meta` event logs `recordingStartEpochMs` (recorder side) but the bot logs NOTHING for the Deepgram-open moment (transcript t=0), so `recorder_t0 − deepgram_t0` is unobservable post-hoc. (4) Real-meeting metadata (title / real start time) isn't captured either. **Remaining work = a bot-side change** (capture the Deepgram-open epoch in `transcriber.ts` + the MediaRecorder-start epoch in `meetBot.ts` → write a `{meetingId}.metadata.json` sidecar with the computed `recordingOffsetS`), THEN an ingest-side R2 source + sidecar reader, THEN the QA test that a clip lands on the right words. **Blocked on:** `transcriber.ts`/`meetBot.ts` are pre-existing uncommitted leftovers (leave-untouched), no real recording exists yet (needs a live Meet + a human speaking), and R2 creds (local-disk mode so far). The verifiable v2-side half (R2/local source + sidecar offset + deterministic clip-lands-right test) can be built without the bot change when picked up.
...
- **Validated against real Ragas** (`eval/ragas_compare.py`, scores the SAME dumped answers via `pnpm eval:answer --dump` → `_ask_runs.json`, same gpt-4o-mini judge): faithfulness **mine 0.737 vs Ragas 0.639** (mean |diff| 0.10, identical on clear cases — both flag q14=0.00), relevancy **mine 0.912 vs Ragas 0.783** (|diff| 0.16). My judge tracks Ragas on faithfulness but is slightly lenient; Ragas decomposes finer claims (stricter) and uses continuous relevancy (catches the evasive q16 my coarse buckets missed). Both agree on which answers fail. **Install gotcha confirmed:** ragas 0.4.3 AND 0.2.15 both crash on import (`langchain_community.chat_models.vertexai` removed in langchain 0.4.x) — must pin `langchain<0.4` (now in `requirements.txt`).
...
**Step-4 slice notes — eval-gated routing + codex-reviewed hardening (2026-06-24):**
- **Spine-first routing (the lever that started this):** action-item/decision questions route to `search_structured` (the WHAT) and pull `search_transcript` for the WHY/contrast. Meeting scoping is **ls-first**: `list_meetings` is a tolerant, **date-aware** discovery tool (resolves "the July Acme call" to the right same-title meeting via month/year + token overlap; exposes `ambiguous` + ranked candidates), the scoped search tools stay exact, and a **hard cap drops `list_meetings` from the toolset after one call** (a soft nudge didn't stop gpt-4o-mini thrashing on same-title disambiguation). On the *easy* 19-Q corpus this read as a clean win (fact 0.853→0.922, faithfulness 0.779→0.891); the hardened corpus then showed it was partly riding on the corpus having no same-title meetings.
- **`/codex challenge` adversarial review** (session 019ef3ee) drove the hardening program. Its sharp calls, all acted on: (1) the evidence ruler reused the answer's lexical proxy → not independent (added precision/contamination); (2) the faithfulness judge saw metadata-stripped contexts → false "unsupported" flags (now enriched with meeting/date/speaker); (3) the prompt was corpus-shaped ("only a handful") and `resolveMeetingIds` could silently pick among same-title meetings (added ambiguity surfacing + date-awareness); (4) 2-run deltas are noise (ruler now reports per-question std over N runs).
- **Agent-path ruler** `evidenceEval.ts` (`pnpm eval:evidence --runs N`): PRIMARY `context_fact_recall` (expected facts present in *gathered* evidence — tool-agnostic, credits the spine route the chunk-level `retrievalEval` can't see), PLUS **forbidden-fact contamination** (does a `must_not_say` entity leak into evidence — catches the cross-prospect budget confusion), relevant-meeting precision, irrelevant-meeting count, tool-path, tool-calls, latency, and per-question mean±std. Its first design was chunk-timespan-biased and *under*-credited structured routing (q9's decision row sits in an adjacent chunk to its label); diagnosed via the answer eval, then fixed to fact-in-evidence.
- **Bounded `search_structured`** — returns `{ total_matched, returned, truncated, hint, rows }` newest-first (default cap 50 / max 100). Honest cap (reports total + how to narrow), not a silent `LIMIT`. Prevents the context blowout when "everything across all meetings" hits hundreds of meetings; no regression at the current 48-record corpus (under the cap). `harvest()` unwraps the new `{rows}` shape so citations still resolve.
- **Corpus grown 7→9 / golden 19→27:** a **second same-title `sales-acme` call** (July, evolved terms: 10→25 seats, $30k→$45k, annual contract, new SSO/SAML blocker — recency + date-disambiguation) and a **Globex** call (Marco again across companies; "Fireflies" vs Acme's "Otter" for contamination). New Q cover date-scoped, recency-supersession, cross-call aggregation, cross-entity no-conflation, and vague-reference refusal. Adding the 2nd Acme call retroactively made q12/q13/q14 ambiguous → their questions disambiguated to the June call to match their labels.
- **Honest verdict (the real deliverable):** not a single-metric win — a far harder, more honest eval that turns gpt-4o-mini's limits into measurable signal. Final (27 Q): answer fact ~0.69 / faithfulness ~0.87 / grounded 0.96; evidence `context_fact_recall` ~0.72 with **high per-question variance on the Acme date-scoped questions** (std up to 0.47) = a model-capability ceiling, not a prompt bug. Residuals: q25 confabulates instead of refusing (refusal stuck ~0.67); the q17 contamination flag is a keyword false-positive (the rejected-Friday discussion legitimately shares words with its `must_not_say`).
- **NEXT (eval-gated, both billed tool-use so cost-gated):** (a) bump the ask-model to gpt-4o / gpt-5-mini-class to test whether the flaky disambiguation is capability; (b) N=15–20 stable run for a defensible final number. Cheap non-API follow-ups: fix the q17 contamination false-positive; per-call-type token logging.
